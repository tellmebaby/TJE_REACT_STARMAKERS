...
const Read = ({ starNo, board, fileList, isLoading }) => {
  const { session } = useSession(); // 세션 컨텍스트에서 세션 가져오기
  ...
  return (
    <div className="container2">
      ...
      <div className="d-flex justify-content-end mt-2 button-box">
        ...
        {session.user && session.user.userNo === board.userNo && (
          <>
      ...
      <div className="reply-container">
        <div className="reply-box">
          <textarea value={replyContent} onChange={(e) => setReplyContent(e.target.value)} placeholder="자유롭게 의견을 작성하세요. 운영원칙에 위배되는 댓글은 삭제될 수 있습니다. 로그인 하신 분만 댓글을 작성할 수 있습니다." disabled={!session.user}></textarea>
          <button type="button" className="btn-reply" onClick={handleReplySubmit} disabled={!session.user}>등록</button>
        </div>
      </div>
            ...
            <div key={comment.id} className="reply">
              {editingCommentId === comment.id ? (
                ...
                <>
                  ...
                  {session.user && session.user.userNo === comment.writerNo && (
